...
import type { Book, ID, Publication, PublicationDraft } from '@/types';
...
function toPicks(p: Publication): BookPick[] {
  return p.recommendedBooks.map((b) => ({ bookId: b.bookId, note: b.note }));
}
...
export default function EditPublicationScreen() {
  ...
  const { data, isLoading, error } = usePublication(id ?? '', isAdmin && !!id);
  ...
  const canSave = title.trim().length > 0 && content.trim().length > 0 && !update.isPending && id;
  ...
  async function handleSave() {
    if (!id) return;
    const draft: Partial<PublicationDraft> & { id: string } = {
      id,
      title: title.trim(),
    ...
    try {
      ...
      router.replace(`/post/${saved.id}` as never);
    } catch {
      Alert.alert(t('common.error'), t('common.tryAgain'));
    }
  }
  ...
  if (isLoading || !data) {
    return (
      <RoleGate role="admin">
        <>
          <AppHeader back title={t('posts.editTitle')} />
          <Screen>
            <Section>
    ...
    return (
      <RoleGate role="admin">
        <>
          ...
          <Screen>
            <Section>
              <EmptyState title={t('common.error')} hint={t('common.tryAgain')} />
            </Section>
          </Screen>
        </>
      </RoleGate>
            ...
            <Card level={0} style={{ gap: theme.spacing.md }}>
                    ...
                    return (
                      ...
                      >
                        ...
                        <Chip
                          ...
                          selected={picked}
                          icon={
                            picked ? <Check size={12} color={theme.colors.primary} /> : undefined
                          }
              ...
              {selectedBooks.length > 0 ? (
                <View style={{ gap: theme.spacing.md, marginTop: theme.spacing.sm }}>
                  {selectedBooks.map((p) => {
                    const book =
                      searchBooks.find((b) => b.id === p.bookId) ??
                      (data.recommendedBooks.find((rb) => rb.bookId === p.bookId)?.book as
                        | Book
                        | undefined);
                    return (
                      <View
                        ...
                        <View style={{ flex: 1, gap: theme.spacing.xs }}>
                          ...
                          >
                            <View style={{ flex: 1, minWidth: 0 }}>
                              ...
                              {book && 'authorName' in book ? (
                                <Text variant="caption" color="fgSubtle" numberOfLines={1}>
                                  {(book as Book).authorName}
                                </Text>
                              ) : null}
                            </View>
                            <IconButton
